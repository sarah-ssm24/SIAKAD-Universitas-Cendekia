...
app.post('/api/mahasiswa', async (c) => {
    try {
        ...
        const [res] = await pool.query('INSERT INTO mahasiswa (nama_mahasiswa, angkatan, semester_ke, id_departemen) VALUES (?, ?, ?, ?)', [nama_mahasiswa, angkatan, semester_ke, id_departemen]);
        return c.json({ success: true, id: res.insertId }, 201);
    }
    catch (e) {
        return handleError(c, e);
...
app.put('/api/mahasiswa/:id', async (c) => {
    try {
        const { nama_mahasiswa, angkatan, id_departemen } = await c.req.json();
        const semester_ke = await getSemesterKeForAngkatan(Number(angkatan));
        await pool.query('UPDATE mahasiswa SET nama_mahasiswa=?, angkatan=?, semester_ke=?, id_departemen=? WHERE NRP=?', [nama_mahasiswa, angkatan, semester_ke, id_departemen, c.req.param('id')]);
        return c.json({ success: true });
    }
    catch (e) {
        return handleError(c, e);
...
app.post('/api/matakuliah', async (c) => {
    try {
        ...
        const [res] = await pool.query('INSERT INTO mata_kuliah (nama_matkul, sks) VALUES (?, ?)', [nama_matkul, sks]);
        return c.json({ success: true, id: res.insertId }, 201);
    }
    catch (e) {
        return handleError(c, e);
